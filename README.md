...
**Step 2 — Generate session config**

```bash
ystar setup
```

This creates `.ystar_session.json` in your project directory.
Answer the interactive questions: project name, paths to block, commands to block, SLA timeouts.

**Step 3 — Register the OpenClaw hook**

```bash
ystar hook-install
```

This writes the `PreToolUse` hook into `~/.claude/settings.json`.
Restart OpenClaw after this step.
...
Y\*gov is now active. Every tool call your agent makes will be checked against this contract.

---

## Set up automatic data sync (optional but recommended)

This script syncs your CIEU audit data to GitHub every 15 minutes,
so your team can monitor and analyze governance activity in real time.
...
- The directory where `.ystar_cieu.db` lives (your project directory)
- How often to sync (default: every 15 minutes)
- A label for your machine (e.g. `mac-test`)

After setup, your CIEU data syncs automatically to the `telemetry` branch of this repository.
...
Action blocked. CIEU chain records the attempt. Agent receives a clear message and can retry.
...
Every decision is written as a 5-tuple:
...
SHA-256 hash chain. Append-only. Any historical decision can be replayed exactly
from the CIEU snapshot at that moment — fully auditable, legally defensible.

---

## Path A — Self-evolving governance

Y\*gov includes **Path A**, a deterministic causal reasoning agent that improves
the governance system itself over time.
...
The "intelligence" grows naturally from CIEU data.
Same CIEU history, same decision — always.
Every Path A action is itself governed by `PATH_A_AGENTS.md`
and its SHA-256 hash is verified on every cycle.
...
Use K9Audit for function-level causal tracing with precise before/after state capture.
...
- CIEU five-tuple causal evidence structure (Y\*_t, s_t, a_t, s_{t+1}, ε_t)
