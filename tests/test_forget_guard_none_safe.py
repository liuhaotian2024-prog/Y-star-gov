...
class TestForgetGuardNoneSafe:
    ...
    def test_matches_pattern_none_pattern(self, tmp_path):
        """Legacy pattern rules now hard-fail under structured-only schema."""
        with pytest.raises(ForgetGuardSchemaError):
            _make_guard_with_rules([
                {
    ...
    def test_matches_pattern_empty_pattern(self, tmp_path):
        """Empty legacy patterns are also rejected by schema validation."""
        with pytest.raises(ForgetGuardSchemaError):
            _make_guard_with_rules([
                {
    ...
    def test_matches_pattern_regex_with_none_context_values(self, tmp_path):
        """Regex-style speech matching is forbidden in v0.5 structured-only mode."""
        with pytest.raises(ForgetGuardSchemaError):
            _make_guard_with_rules([
                {
                    "name": "test_regex",
    ...
    def test_load_rules_null_pattern_yaml(self, tmp_path):
        """Mixed legacy pattern YAML should fail fast instead of coercing patterns."""
        with pytest.raises(ForgetGuardSchemaError):
            _make_guard_with_rules([
                {
    ...
    def test_schema_11_rule_no_keywords(self, tmp_path):
        """Pre-v0.5 schema 1.1 rules should fail explicit structured-only validation."""
        with pytest.raises(ForgetGuardSchemaError):
            _make_guard_with_rules([
                {
