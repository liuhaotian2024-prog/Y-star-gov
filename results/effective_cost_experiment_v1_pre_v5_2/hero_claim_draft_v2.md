...
- Mechanical verification showed only **78%** of those baseline artifacts actually pass the task spec.
- **Deception rate** = 89% claimed − 78% verified = **11%** of baseline trials are "looks right, fails verifier".
- With Trampoline: verified rate rose to **100%**.
- Effective cost per real completion: baseline $0.0264, Trampoline $0.0258 (+2%).

## Per-provider breakdown (the cross-provider story)
- **claude-opus-4-7**: baseline verified 100% → Trampoline 100% (Δ +0pp)
- **claude-sonnet-4-6**: baseline verified 78% → Trampoline 100% (Δ +22pp)
...
5. **Trampoline cost outlier**: 0/27 Trampoline trials
...
- **Claim 1** (frontier agents fake-complete, cross-provider): **partially
  supported**. Sonnet shows 22% deception on the scout-run
