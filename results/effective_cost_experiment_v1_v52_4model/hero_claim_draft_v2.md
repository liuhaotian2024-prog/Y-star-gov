...
## Empirical headline (filled-in from 72 scout trials)

Across **72 runs** on **claude-opus-4-7, claude-sonnet-4-6, deepseek-chat, gpt-5** and 3 coding tasks
(cross_file_refactor / test_gen_for_existing / lint_fix):

- Agents **emitted a solution artifact** (= claimed completion by code-block emission) on **92%** of baseline trials.
- Mechanical verification showed only **81%** of those baseline artifacts actually pass the task spec.
- **Deception rate** = 92% claimed − 81% verified = **11%** of baseline trials are "looks right, fails verifier".
- With Trampoline: verified rate rose to **97%**.
- Effective cost per real completion: baseline $0.0305, Trampoline $0.0439 (-44%).

## Per-provider breakdown (the cross-provider story)
- **claude-opus-4-7**: baseline verified 89% → Trampoline 100% (Δ +11pp)
- **claude-sonnet-4-6**: baseline verified 67% → Trampoline 89% (Δ +22pp)
- **deepseek-chat**: baseline verified 67% → Trampoline 100% (Δ +33pp)
- **gpt-5**: baseline verified 100% → Trampoline 100% (Δ +0pp)
...
5. **Trampoline cost outlier**: 0/36 Trampoline trials
   ran ≥10 iterations and consumed **0% of total Trampoline
