...
def compute_avg_age(users: List[Dict[str, Any]]) -> float:
    ages: List[float] = [u['age'] for u in users if 'age' in u]
    if not ages:
        return 0.0
    return sum(ages) / len(ages)


def count_by_domain(emails: List[str]) -> Dict[str, int]:
    counts: Dict[str, int] = {}
    for e in emails:
        if '@' not in e:
            continue
