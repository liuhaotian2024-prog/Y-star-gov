# trial_id: 802172bd8216
# (arm name redacted for blind review)

# === data_processor.py ===
from typing import List, Dict, Iterable, Any

def filter_active_users(users: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [u for u in users if u.get('active')]


def compute_avg_age(users: Iterable[Dict[str, Any]]) -> float:
    # Explicitly cast age to float to ensure type safety for arithmetic operations in strict mode.
    ages = [float(u['age']) for u in users if 'age' in u]
    if not ages:
        return 0.0
