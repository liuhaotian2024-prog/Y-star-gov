...
def test_clean_records_drops_invalid():
    recs = [
        {'name': 'A', 'email': 'a@x.com', 'age': 1},
        {'name': 'B', 'email': 'b@y.com'},  # missing age
        {'name': 'C', 'email': 'c@z.com', 'age': 'oops'},  # wrong type
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 1
    assert out[0]['email'] == 'a@x.com'
...
def test_clean_records_drops_empty_email():
    recs = [
        {'name': 'A', 'email': '   ', 'age': 1},
        {'name': 'B', 'email': 'b@y.com', 'age': 2},
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 1
    assert out[0]['email'] == 'b@y.com'


def test_clean_records_deduplicates_by_email():
    recs = [
        {'name': 'A', 'email': 'a@x.com', 'age': 1},
        {'name': 'A2', 'email': 'A@X.COM', 'age': 2},
        {'name': 'B', 'email': 'b@y.com', 'age': 3},
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 2
    assert out[0]['name'] == 'A'
...
def test_aggregate_by_domain_unknown():
    recs = [{'email': 'noatsign'}, {'email': 'a@x.com'}]
...
def test_pipeline_end_to_end(tmp_path):
    ...
    data = [
        ...
        {'name': 'Dup', 'email': 'a@x.com', 'age': 9},
        {'name': 'Bad', 'email': 'bad@y.com'},  # missing age
...
def test_pipeline_empty_list(tmp_path):
    p = tmp_path / 'empty.json'
    p.write_text('[]', encoding='utf-8')
