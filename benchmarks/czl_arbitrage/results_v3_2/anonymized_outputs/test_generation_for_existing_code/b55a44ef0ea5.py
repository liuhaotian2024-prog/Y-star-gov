...
def test_clean_records_skips_invalid():
    ...
    records = [
        {"email": "a@b.com", "name": "Alice"},
        {"email": "bad", "name": "Bob"},          # missing name? no, email is valid but name present – actually valid
        {"name": "NoEmail"},                       # missing email -> ValidationError
        {"email": 123, "name": "WrongType"},       # wrong type for email -> ValidationError
        {"email": "   ", "name": "EmptyEmail"},    # empty after strip -> ValueError
    ]
    result = clean_records(records, schema)
    # Only first record is valid
    assert len(result) == 1
    assert result[0]["email"] == "a@b.com"
