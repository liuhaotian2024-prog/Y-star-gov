...
def test_load_records_top_level_string(tmp_path):
    p = tmp_path / 'str.json'
...
def test_clean_records_happy():
    recs = [
        ...
        {'name': 'B', 'email': 'b@y.com', 'age': 2},
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 2
    assert out[0]['email'] == 'a@x.com'
    assert out[1]['email'] == 'b@y.com'


def test_clean_records_drops_duplicates_by_email():
    recs = [
        {'name': 'A', 'email': 'dup@x.com', 'age': 1},
        {'name': 'A2', 'email': 'DUP@x.com', 'age': 2},
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 1
    assert out[0]['email'] == 'dup@x.com'


def test_clean_records_skips_invalid():
    recs = [
        {'name': 'A', 'email': 'a@x.com', 'age': 1},
        {'name': 'B', 'email': 'b@x.com'},  # missing age
        {'name': 'C', 'email': 123, 'age': 3},  # wrong type
        {'name': 'D', 'email': '   ', 'age': 4},  # empty email after normalize
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 1
    assert out[0]['email'] == 'a@x.com'
...
def test_clean_records_does_not_mutate_input():
    rec = {'name': 'A', 'email': 'A@X.com', 'age': 1}
...
def test_aggregate_by_domain_unknown_for_missing_at():
    recs = [{'email': 'noatsign'}, {'email': 'also-bad'}]
    assert aggregate_by_domain(recs) == {'unknown': 2}
...
def test_aggregate_by_domain_missing_key_raises():
    with pytest.raises(KeyError):
        aggregate_by_domain([{'no_email': 1}])
...
def test_pipeline_end_to_end(tmp_path):
    data = [
        ...
        {'name': 'C', 'email': 'c@y.com', 'age': 3},
        {'name': 'D', 'email': 'a@x.com', 'age': 4},  # duplicate
        {'name': 'E', 'email': 'e@z.com'},  # invalid - skipped
    ]
    p = tmp_path / 'data.json'
    p.write_text(json.dumps(data), encoding='utf-8')
    counts = pipeline(str(p), SCHEMA)
    assert counts == {'x.com': 2, 'y.com': 1}


def test_pipeline_missing_file_propagates(tmp_path):
    with pytest.raises(FileNotFoundError):
