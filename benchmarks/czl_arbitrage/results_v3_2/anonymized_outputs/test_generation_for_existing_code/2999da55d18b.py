...
def test_clean_records_skips_invalid_missing_field():
    schema = {"email": str, "name": str}
    records = [
        {"email": "a@b.com", "name": "Alice"},
...
def test_clean_records_skips_invalid_wrong_type():
    schema = {"email": str, "age": int}
    records = [
        {"email": "a@b.com", "age": 25},
        {"email": "c@d.com", "age": "old"},
    ]
    result = clean_records(records, schema)
    assert len(result) == 1
...
def test_clean_records_all_invalid():
    schema = {"email": str}
    records = [{"email": ""}, {"email": "   "}, {"name": "no-email"}]
    result = clean_records(records, schema)
    assert result == []
