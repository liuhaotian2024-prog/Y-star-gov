...
def test_load_records_happy(tmp_path):
    ...
    assert load_records(str(p)) == payload
...
def test_clean_records_skips_invalid():
    recs = [
        {"name": "A", "email": "a@x.com", "age": 1},      # ok
        {"name": "B", "email": "b@x.com"},                # missing age
        {"name": "C", "email": "c@x.com", "age": "x"},    # wrong type
        {"name": "D", "email": "   ", "age": 4},          # empty email after strip
    ]
    out = clean_records(recs, SCHEMA)
    assert len(out) == 1
    assert out[0]["email"] == "a@x.com"
...
def test_aggregate_by_domain_missing_email_raises():
...
def test_pipeline_bad_json(tmp_path):
    p = tmp_path / "bad.json"
    p.write_text("not json", encoding='utf-8')
    with pytest.raises(ValueError):
